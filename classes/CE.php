...
class CE {
	...
	public static function init( $args = [] ) {
		...
		self::$options = self::arrayMerge( self::$options, $args );
	...
	public static function getStyleFromArray( $array ) {
		...
		foreach ( $array as $property => $value ) {
			...
			if ( ! is_array( $value ) ) {
				...
				$response .= self::getStyleFromArray( $value );
			}
		}
	...
	public static function __callStatic($name, $arguments) {
		if ($nameComponent = strripos($name, 'the') === 0) {
			$type = 'the';
		} else if($nameComponent = strripos($name, 'get') === 0) {
			$type = 'get';
		} else {
			return false;
		}
	...
	public static function load_vue() {
		wp_enqueue_script( 'vendors-js', 'https://unpkg.com/vue@/' . self::$options['vue_version'], false, false,
		                   true );
	}
	...
	public static function arrayMerge( $array1, $array2 ) {
		...
		foreach ( $array1 as $key => $item ) {
			if ( is_array( $item ) && isset( $array2[ $key ] ) ) {
				$bufer[ $key ] = self::arrayMerge( $item, $array2[ $key ] );
		...
		while ( $query->have_posts() ) {
			...
			if ($args['thumbnail']) {
				...
				if ( $thumbnail_id ) {
					...
					$thumbnail['url'] = wp_get_attachment_image_url( $thumbnail_id, 'full' );
					$thumbnail['alt'] = get_post_meta( $thumbnail_id, '_wp_attachment_image_alt', true );
					foreach ( $sizes as $size ) {
						$thumbnail[$size] = wp_get_attachment_image_url( $thumbnail_id, $size );
					}
				}
			}

			if ($thumbnail) {
				$newParams['thumbnail'] = $thumbnail;
			}

			$newParams['post'] = $post;
			$newParams['link'] = get_post_permalink($post);


			$itemParams = self::arrayMerge( $bufer, $newParams);
